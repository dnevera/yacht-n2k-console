#!/usr/bin/env bash
# deploy_dashboard.sh — push ha/ais/build/dashboard-ais.yaml into the live
# Home Assistant instance's storage-mode Lovelace config (Stage or Prod),
# as its OWN dashboard key (isolated from ha/sailing-dash's).
#
# USAGE
...
#
# Mirrors ha/sailing-dash/helpers/deploy_dashboard.sh: idempotent (semantic
# YAML diff against the live config decides whether anything is uploaded at
# all), backs up the previous document, restarts HA only when something
# really changed.
set -euo pipefail
...
YAML_FILE="${SCRIPT_DIR}/build/dashboard-ais.yaml"
STORAGE_KEY="lovelace.dashboard_ais"
...
echo "== AIS dashboard deploy (profile: ${TARGET_ENV}) → ${HA_HOST} (container: ${HA_CONTAINER}, transport: ${HA_TRANSPORT}) =="

# build.py runs once per pipeline in deploy.sh; when this script is used
# standalone, compile the artifact it needs here.
if [[ "${AIS_BUILD_DONE:-0}" != "1" ]]; then
    echo "== Running build.py =="
    python3 "${HELPERS_DIR}/build.py"
fi

# ── 1. Convert source of truth (YAML) into the .storage JSON document ───────
TMP_JSON="$(mktemp /tmp/lovelace_dashboard_ais.XXXXXX)"
LIVE_AS_YAML="$(mktemp /tmp/lovelace_dashboard_ais_live.XXXXXX)"
...
echo "--- pre-deploy diff (live HA config vs local build/dashboard-ais.yaml) ---"
...
# ── 4. Restart HA ────────────────────────────────────────────────────────────
CHANGED_EARLIER=0
if [[ -n "${AIS_CHANGE_FLAG:-}" && -s "${AIS_CHANGE_FLAG}" ]]; then
    CHANGED_EARLIER=1
fi
...
echo "Done. AIS dashboard updated in container ${HA_CONTAINER}."
