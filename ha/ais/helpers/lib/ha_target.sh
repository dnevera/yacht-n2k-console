#!/usr/bin/env bash
# ha_target.sh — the ONE access layer to a Home Assistant target, for ha/ais.
#
# This is a thin copy of ha/sailing-dash/helpers/lib/ha_target.sh, kept in
# sync deliberately rather than shared by import: the two dashboard packages
# must stay independently deployable (no cross-package sourcing), but the
# access-layer logic itself (transport, idempotent sha256-diff delivery,
# restart) must never drift between them. If you change this file, check
# whether the same fix applies to the sailing-dash copy too.
#
# Targets are NAMED PROFILES of the same abstraction, declared in
# ha/ais/.env (see .env.template), differing only in transport (local
# docker vs ssh + docker) — never in code path. Source this file and call
# `ha_target_init <profile> [user@host]`, then use:
#
...
# lib/ lives in ha/ais/helpers/lib/ — the subproject root is two up.
HA_TARGET_SCRIPT_DIR="$(cd "${HA_TARGET_LIB_DIR}/../.." && pwd)"
HA_TARGET_PROJECT_ROOT="$(cd "${HA_TARGET_SCRIPT_DIR}/../.." && pwd)"

# The profile itself comes from ha/ais/.env ONLY. Neither the repo root
# deploy.conf nor ha/sailing-dash/.env are ever sourced here.
...
# Delivers a whole directory (e.g. custom_components/ais_targets) into the
# container: `docker cp` locally, `scp` + `docker cp` over SSH.
...
#
# Both *_if_changed helpers return 0 when they copied something and 1 when
# they skipped, and keep the counters HA_DELIVERED / HA_SKIPPED so a caller
# can decide whether a container restart is needed at all.
HA_DELIVERED=0
HA_SKIPPED=0

# Deploy state file for this package specifically — deliberately a DIFFERENT
# path than sailing-dash's (ais_deploy_state vs sailing_deploy_state), so the
# two packages' tree-manifest bookkeeping can never collide inside the same
# container.
HA_STATE_PATH="/config/.storage/ais_deploy_state"
...
ha_state_load() {
    HA_STATE_FILE="$(mktemp -t ais_deploy_state)"
...
ha_state_set() {
    ...
    local tmp
    tmp="$(mktemp -t ais_deploy_state_new)"
