...
Единственный скролл-контейнер таблицы — её `ha-card` (`overflow: auto` по **обеим** осям, `max-height` из числа видимых строк). Прежняя схема, где вертикально скроллился `tbody` (`display: block`), ломалась при горизонтальном скролле: вертикальный ползунок оказывался далеко справа за пределами видимой области. Заголовок держится на месте через `thead th { position: sticky; top: 0 }` с фоном `var(--card-background-color)`. Ячейки и заголовки получили `padding: 6px 10px` — раньше контент прилипал к краям карточки. `table-layout: fixed` больше не используется: ширина колонки имени задаётся `min-width` (`table.vessel_column_width`), а общая минимальная ширина — `table.min_width_px`.
