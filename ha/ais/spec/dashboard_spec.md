# Спецификация дашборда и интерфейса AIS

## Обзор

Дашборд AIS — отдельный Lovelace-вид Home Assistant (`lovelace.dashboard_ais`), состоящий из карты со всеми целями AIS и своим судном, а также полной кликабельной таблицы деталей по каждой цели. Дашборд собирается статически из YAML-исходников пайплайном `helpers/build.py` и деплоится идемпотентно скриптом `deploy.sh`.

## Структура исходников

```
ha/ais/src/yaml/dashboard/
├── header.yaml                       — заголовок вида (title, icon, max_columns)
└── sections/
    └── 01_ais_map.yaml               — карта + таблица целей (единственная секция)
```

## Пайплайн сборки (`helpers/build.py`)

```mermaid
graph LR
    H[header.yaml] --> B[build_dashboard]
    S[sections/01_ais_map.yaml] --> B
    B --> OUT[build/dashboard-ais.yaml]
```

### Единственный источник правды — YAML-шаблоны

`src/yaml/dashboard/header.yaml` и `src/yaml/dashboard/sections/*.yaml` — **единственный источник правды** для структуры и стилей: раскладка, геометрия оверлея, набор колонок таблицы, все стили `card_mod`/`css`.

`helpers/build.py` делает ровно две вещи:

1. подставляет явные плейсхолдеры `${AIS_*}`, объявленные самими шаблонами, значениями из `config.yaml` (дефолты — в `config.yaml.template`); неизвестный плейсхолдер — ошибка сборки, а не тихое игнорирование;
2. склеивает `header.yaml` + `sections/*.yaml` (в алфавитном порядке) в `views[0].cards` и пишет `build/dashboard-ais.yaml` (одноразовый артефакт, руками не правится).

Скрипт **не имеет права** ничего генерировать, дописывать или «нормализовать» внутри карточек. Ранняя версия генерировала целые `card_mod`-блоки и колонки таблицы из `config.yaml` и молча затирала содержимое шаблонов: любая ручная правка в `src/yaml/**` исчезала при следующем деплое (симптом — «отредактировал локально, задеплоил, а на stage старая карточка»). Возвращать эту логику нельзя.

### Опции `config.yaml` и их плейсхолдеры

| Ключ | Плейсхолдер | Назначение |
|---|---|---|
| `map.default_zoom` | `${AIS_MAP_ZOOM}` | начальный зум карты |
| `map.height` | `${AIS_MAP_HEIGHT}` | высота карты (CSS-длина, по умолчанию `calc(100vh - 104px)`) |
| `table.default_sort` | `${AIS_SORT_BY}` | сортировка таблицы по умолчанию (напр. `state+`) |
| `table.visible_rows` × `row_height_px` | `${AIS_ROWS_SCROLL_PX}` | сколько строк видно в полной таблице до скролла |
| `table.collapsed_visible_rows` × `row_height_px` | `${AIS_ROWS_SCROLL_COMPACT_PX}` | то же для компактного сайдбара |

### Важное архитектурное ограничение

`own_mmsi` **намеренно не читается** в `build.py` — единственный источник правды для этого значения — конфигурация интеграции `ais_targets` (Config Flow), а не build-time YAML. Это устраняет риск рассинхронизации между временем сборки дашборда и временем выполнения интеграции (см. `integration_spec.md`).

## Конфигурация вида (`header.yaml`)

```yaml
views:
  - type: panel
    title: AIS
    icon: mdi:radar
    cards:
```

`type: panel` — единственный тип вида Lovelace, который рендерит контент по-настоящему от края до края (без центрированной колонки с max-width, как в `sections`/`masonry`). Вид показывает ровно **одну** карточку верхнего уровня, поэтому карта и таблица завёрнуты в единый `vertical-stack`. Высота карты фиксируется через `card_mod`, чтобы «во всю ширину» не означало «во весь экран по высоте».

## Раскладка: карта + таблица-оверлей

```mermaid
graph LR
    VS[vertical-stack] --> M[map 480px]
    VS --> C1[conditional off: side-bar]
    VS --> C2[conditional on: full table]
```

- В нормальном потоке лежит **только карта** (высота ограничена вьюпортом, полная ширина контента).
- Список целей рендерится **оверлеем** поверх правой части карты: каждая карточка оверлея позиционируется своим собственным `card_mod` (`:host { position: absolute; top/right; z-index }`), заданным прямо в шаблоне.
- Два взаимоисключающих `conditional`-карточки по состоянию `input_boolean.ais_table_expanded`:
  - **`off` (по умолчанию)** — компактный сайдбар (ширина из `table.collapsed_width`): `Vessel`, `Dist (km)` и `SOG (kn)`.
  - **`on`** — полная таблица со всеми колонками, ширина из `table.width`.

### Габариты оверлея: не шире и не выше окна

Ширина обоих оверлеев задаётся **точно так же, как высота карты** — произвольной CSS-длиной в `config.yaml`:

| Опция | Плейсхолдер | По умолчанию |
| :--- | :--- | :--- |
| `table.width` | `${AIS_TABLE_WIDTH}` | `calc(50vw - 48px)` |
| `table.collapsed_width` | `${AIS_TABLE_COLLAPSED_WIDTH}` | `300px` |
| `table.vessel_column_width` | `${AIS_VESSEL_COL_WIDTH}` | `220px` |

По высоте оверлеи по-прежнему растягиваются **сторонами** (`top: 84px; bottom: 12px`), а не явным `height`, и прижаты к правому краю (`right: 12px`); дополнительный `max-width: calc(100vw - 48px)` гарантирует, что любая заданная ширина не выйдет за 12px-отступы окна.

Контейнер (`custom:mod-card` с `margin: 12px`) сам уже отступает от краёв окна, поэтому оверлей физически не может стать шире или выше окна и всегда сохраняет отступы. Явные `width: calc(100vw - 96px)` / `height: calc(100vh - 96px)` из прежней версии этого не гарантировали: они считались от вьюпорта, а не от контейнера, и при смещении `top: 84px` таблица выходила за нижнюю границу.

### Автоматический горизонтальный скролл

- На внутреннем `ha-card` оверлея задано `max-height: 100%; overflow: auto` — это и даёт скролл по обеим осям.
- В `css` полной таблицы добавлено `"table+": " min-width: ${AIS_TABLE_MIN_WIDTH}px;"` (по умолчанию 1100, опция `table.min_width_px`). Без `min-width` таблица с `width: 100%` просто сжималась, а из-за `white-space: nowrap` содержимое ячеек обрезалось; теперь при нехватке ширины включается горизонтальный скролл.
- Вертикальный скролл тела таблицы дополнительно ограничен вьюпортом: `max-height: min(${AIS_ROWS_SCROLL_PX}px, calc(100vh - 260px))`.
- Переключатель разворачивания — карточка `entities` с самим `input_boolean.ais_table_expanded` в шапке оверлея (присутствует в обоих состояниях). Хелпер создаётся идемпотентно скриптом `helpers/provision_helpers.py` в `.storage/input_boolean` при `deploy.sh --install`.
- **Важно про имя хелпера:** Home Assistant формирует `entity_id` из *отображаемого имени*, а не из `id`, поэтому имя обязано быть `AIS table expanded` → `input_boolean.ais_table_expanded`. Ранняя версия использовала имя `AIS Targets`, из-за чего создавался `input_boolean.ais_targets`, оба `conditional` никогда не срабатывали и **оверлей был полностью невидим**. Переименование само по себе не лечит уже зарегистрированный хелпер (его `entity_id` закреплён в `core.entity_registry`), поэтому `provision_helpers.py --entity-registry` удаляет неверную запись, и HA пересоздаёт её корректно при старте.
- Свёрнутое состояние проверяется через `state_not: "on"` (а не `state: "off"`), чтобы сайдбар отображался и в момент, когда хелпер ещё недоступен.
- **Отступы и высота.** Внешний `card_mod` добавляет `margin: 12px` (у `panel`-вида нет собственных отступов, без этого карта прилипала к краям окна), а высота карты ограничивается вьюпортом (`calc(100vh - 104px)`) — фиксированная высота делала страницу выше окна и включала вертикальный скролл браузера. Все эти значения заданы в шаблоне `01_ais_map.yaml`.

## Карточка карты (`type: map`)

```yaml
- type: map
  entities:
    - entity: device_tracker.nevera
  geo_location_sources:
    - ais_targets
  default_zoom: 12
  card_mod:
    style: |
      :host { display: block; height: calc(100vh - 104px); }
      ha-card { height: 100%; }
      #root { padding-bottom: 0 !important; height: 100% !important; }
      ha-map { height: 100% !important; }
```

- **`entities: [device_tracker.nevera]`** — GPS-трекер собственной лодки (fallback, работает одновременно с AIS-целью собственного судна).
- **`geo_location_sources: ['ais_targets']`** — все AIS-цели, включая собственное судно (единый `source`, см. `integration_spec.md`).
- **`card_mod.style`** — фиксирует высоту карточки и внутреннего DOM-элемента `ha-map` (значение задано в шаблоне).

## Таблица целей (`custom:flex-table-card`)

### Почему именно так

| Подход | Проблема |
|---|---|
| Нативная markdown-таблица (Jinja) | DOMPurify/marked.js фильтрует инлайн-`onclick`, клик по строке не работал; отсутствие trim-модификаторов в Jinja ломало GFM-таблицу. |
| Нативная карточка `entities` | Только один state + одна вторичная строка на сущность — не хватает колонок. |
| `custom:auto-entities` + `custom:flex-table-card` | `auto-entities` при **каждом** изменении набора целей заново вызывал `setConfig()` внутренней карточки, из-за чего пересоздавалась таблица и **сбрасывалась сортировка**, выбранная пользователем кликом по заголовку. |
| `custom:flex-table-card` напрямую (текущее решение) | Карточка сама умеет `entities: include: geo_location.ais_*` (wildcard-регексп внутри `_getEntities()`), поэтому `setConfig()` вызывается один раз, а сортировка живёт в экземпляре и **сохраняется**. |

### Конфигурация

```yaml
- type: custom:flex-table-card
  entities:
    include: geo_location.ais_*
  strict: true
  clickable: true
  sort_by: state+
  columns: [... см. ниже ...]
```

- **`entities.include`** — маска по `entity_id`: новые/исчезающие цели подхватываются автоматически.
- **`strict: true`** — скрывает строки с пустой ячейкой, то есть «призраков» — восстановленные (`restored`/`unavailable`) записи реестра без атрибутов, оставшиеся от прежних версий интеграции.
- **`clickable: true`** — клик по строке открывает нативный more-info выбранной цели с мини-картой (центрирование на судне).
- **`sort_by: ${AIS_SORT_BY}`** — сортировка по умолчанию из `config.yaml` (штатно `state+` — по расстоянию, state сущности = дистанция в км); клик по заголовку меняет сортировку и она сохраняется до перезагрузки страницы.
- **Скролл вместо обрезания.** Видно `table.visible_rows` строк, остальные доступны прокруткой: `css.tbody` делает тело таблицы собственным скролл-контейнером (`display: block; max-height; overflow-y: auto`), а `thead, tbody tr { display: table; table-layout: fixed }` сохраняет выравнивание колонок. Штатный `max_rows` карточки **не используется** — он обрезает лишние цели, а не скроллит их.
- **Однострочные ячейки.** `css` дописывает (суффикс `+` — режим append к дефолтному стилю карточки) `white-space: nowrap` для `tr td`/`th`, чтобы длинные имена судов не переносились на вторую строку.
- **Своя лодка** — обычная строка таблицы с тем же `geo_location.ais_<mmsi>`; отличается только иконкой ⛵ в начале имени (без подписи «Own Boat»), которую ставит интеграция при совпадении MMSI с `own_mmsi`.

### Колонки таблицы (заданы в шаблоне)

Список колонок целиком лежит в `sections/01_ais_map.yaml` — добавление/удаление/переупорядочивание колонок делается там. `data` — **плоский** ключ (не путь `attributes.x`): карточка резолвит `name`/`state`/... специальным образом, затем как член сущности, затем как `entity.attributes[key]`.

| Колонка | `data` |
|---|---|
| Vessel | `name` |
| Dist (km) | `state` |
| SOG (kn) | `sog` — сразу после расстояния (скорость цели читается вместе с дистанцией) |
| MMSI | `mmsi` |
| Callsign | `callsign` |
| Type | `ship_type` |
| Length (m) | `length` |
| Beam (m) | `beam` |
| Destination | `destination` |
| COG (°) | `cog` |
| Heading (°) | `heading` |
| Nav Status | `nav_status` |
| Updated | `last_seen` — **локальное** время `HH:MM:SS` (машиночитаемый UTC ISO остаётся в `last_seen_iso`) |

## Зависимости Lovelace-карточек

| Карточка | Репозиторий | Способ получения |
|---|---|---|
| `custom:flex-table-card` | `custom-cards/flex-table-card` | `github_raw`, версия зафиксирована в `ha/sailing-dash/deps.yaml` |
| `custom:auto-entities` | `thomasloven/lovelace-auto-entities` | `github_raw`; дашбордом больше не используется, но ресурс остаётся зарегистрированным |
| `card-mod` | — | нужен для оверлея и фиксированной высоты карты; уже присутствует на инстансе из `sailing-dash` |

Зависимости пинятся по версии/тегу в `ha/sailing-dash/deps.yaml` (ресурсы Lovelace общие для инстанса HA), а `ha/ais/deploy.sh` сам скачивает и регистрирует их (`deploy_card_deps()`).

## Связанные документы

- [`architecture.md`](./architecture.md) — общая архитектура и обоснование дизайна.
- [`integration_spec.md`](./integration_spec.md) — источник атрибутов, отображаемых в таблице.
- [`deployment_and_ops.md`](./deployment_and_ops.md) — деплой карточек и дашборда.

### Почему оверлей обёрнут в `custom:mod-card`

**card-mod не умеет стилизовать stack-карточки.** В `card-mod.js` нет ни одного упоминания `hui-*-stack-card` — библиотека хукается только на `ha-card`, `hui-card`, `hui-entities-card`, `hui-conditional-row`, `hui-picture-elements-card`, `hui-section`, `hui-view` и т.п. Поэтому `card_mod`, повешенный напрямую на внешний `vertical-stack`, **молча игнорировался**: не появлялись ни внешние отступы (`margin: 12px`), ни позиционирование оверлея (`#root > *:nth-child(2)`) — таблица просто оставалась в обычном потоке под картой.

Актуальная схема:

- Внешний контейнер — `custom:mod-card` (карточка идёт в составе `card-mod.js`): она рендерит собственный настоящий `<ha-card>` вокруг любой вложенной карточки, поэтому `card_mod` к ней применяется гарантированно. На нём заданы `position: relative` (offset-родитель оверлея) и `margin: 12px` (внешние отступы).
- Внутри — обычный `vertical-stack` без какого-либо `card_mod`.
- Карта остаётся в нормальном потоке и задаёт высоту контейнера (`calc(100vh - 104px)`).
- Тумблер и обе таблицы позиционируются `position: absolute` **своим собственным** `card_mod` — это реальные `ha-card`, к которым card-mod применяется штатно. Никакой зависимости от порядка детей (`nth-child`) внутри shadow root стека больше нет, поэтому раскладка не «съезжает» при перерисовке набора целей.
- Вся геометрия задана инлайн в шаблоне `01_ais_map.yaml`.

### Почему оверлеи позиционируются через `:host`, а не через `ha-card`

card-mod внедряет свой `<style>` **в shadow root самой карточки**, поэтому селектор `ha-card` стилизует только внутреннюю обёртку, а сам элемент карточки (`hui-tile-card`, `flex-table-card`) и его контейнер `hui-card` остаются в потоке flex-колонки `vertical-stack` (по исходникам HA: `:host { display:flex; flex-direction:column; height:100% }`, `#root { flex:1 }`). Последствия ровно те, что наблюдались:

- обёртки продолжали резервировать высоту под таблицей → страница становилась выше окна и включался вертикальный скролл браузера;
- карточка переключателя растягивалась колонкой на всю доступную высоту.

Правильный способ — `:host { position: absolute; ... }`: из потока выносится **вся** карточка, обёртка `hui-card` схлопывается в ноль, а внутреннему `ha-card` остаётся только роль скролл-контейнера (`max-height: 100%; overflow: auto`).

Дополнительно:
- переключатель — карточка `tile` (компактная строка с нативным тумблером) вместо громоздкой `entities`;
- высота карты — `calc(100vh - 104px)`: 56px верхней панели + строка табов + оба отступа по 12px, с небольшим запасом (пары лишних пикселей достаточно, чтобы скролл вернулся);
- на контейнере `custom:mod-card` добавлен `overflow: hidden`, чтобы оверлей не мог вылезти за пределы карточки.

### Чому карта була гігантською і сторінка скролилась

`hui-map-card._computePadding()` (джерело HA frontend) виставляє **інлайновий** `#root { padding-bottom: 100% }`, якщо карточка не є прямим дитям panel/grid-розкладки. Наша карта живе всередині `vertical-stack`, тож `layout` не дорівнює `panel` — карта робила себе **квадратною по своїй ширині**: на edge-to-edge панелі це ≈ ширина вікна по висоті, звідси вертикальний скрол браузера. Додатково попередній `card_mod` націлювався на `#map`, якого не існує (елемент називається `ha-map`), тому нічого не застосовувалось.

Робочий стиль карти:
```
:host { display: block; height: calc(100vh - 104px); }
ha-card { height: 100%; }
#root { padding-bottom: 0 !important; height: 100% !important; }
ha-map { height: 100% !important; }
```

### Подпись маркера на карте

По умолчанию `ha-map` подписывает маркер `geo_location` **инициалами** имени (первые буквы слов, максимум 3 символа) — то есть ни имени, ни данных о судне на карте фактически не видно. Читаемую подпись даёт только режим `label_mode: attribute`, поэтому источник карты задан объектом:

```yaml
geo_location_sources:
  - source: ais_targets
    label_mode: attribute
    attribute: map_label
```

Атрибут `map_label` формирует интеграция (`AisTarget._map_label()`) в виде `⛵XX 6.4kn 120m`, где первый символ — **значок типа цели**, `XX` — **двухбуквенный бейдж** из имени судна (`AisTarget._initials()`, как в дефолтном поведении HA). Полное имя на карту намеренно не выводится (маркер превращался в «кляксу»), а недоступные поля просто опускаются — ни `unknown`, ни прочерков на карте быть не должно. Beam в подпись не входит, только длина.

### Кнопка «домой» и зум карты (`map.home_zoom`)

Штатная опция карточки `type: map` — `default_zoom` — на этом дашборде **не работает принципиально**: `hui-map-card` при более чем одной точке всегда вызывает `fitMap()` (`fitBounds()` по всем маркерам), а маркеров всегда много — каждая AIS-цель. Поэтому правка `default_zoom` в конфиге выглядела как «деплой не доехал», хотя значение исправно доходило до `.storage` живого HA. Опция удалена и из шаблона карточки, и из `config.yaml.template`.

Вместо неё введён ключ `map.home_zoom` (плейсхолдер `${AIS_HOME_ZOOM}`). Кнопка «домой» карточки (последняя `ha-icon-button` внутри `div#buttons` в shadow root `hui-map-card`, штатно вызывающая `_resetFocus()` → `fitMap()`) перехватывается в capture-фазе в `src/js/ais-select-bridge.js`: вместо подгона под все цели карта центрируется на **нашей** лодке (`is_own_ship`, MMSI по-прежнему только в конфиг-записи интеграции) с зумом `home_zoom`. Если своей позиции ещё нет, событие не трогается и отрабатывает штатный fit-all.

Значение попадает в JS через конфиг карточки `custom:ais-user-scope` (`home_zoom:`), которая публикует его в `window.__aisHomeZoom`: сам мост деплоится файлом как есть, подстановки плейсхолдеров внутри `.js` не происходит.

### Своя лодка на карте

Своя лодка — обычная AIS-цель с тем же `source`, поэтому на карте она ничем не отличалась от чужих кружков. Отличий теперь два:

1. **Значок своей лодки в подписи маркера.** `AisTarget._map_label()` при `is_own_ship` ставит `_OWN_BOAT_ICON` вместо глифа типа судна (бейдж, скорость и длина остаются как у всех).
2. **Двойной контур акцентным цветом.** Ставит `src/js/ais-select-bridge.js`: раз в 2 с он обходит shadow root'ы, находит `ha-entity-marker` с `entity-id` своей цели и вешает на него класс `ais-own-ship` (стиль `box-shadow` с двумя кольцами — внутреннее цвета фона карточки, внешнее `--ais-own-ship-color` / `--accent-color`), инжектируя `<style>` в тот же shadow root.

**Почему не card_mod-CSS.** CSS-правило пришлось бы адресовать маркер по `entity-id`, то есть **дублировать MMSI** в конфиге дашборда, хотя единственный источник правды — опция `own_mmsi` конфиг-записи интеграции. Скрипт вместо этого находит цель в рантайме по атрибуту `is_own_ship`, поэтому MMSI нигде не повторяется. Опрос по таймеру нужен потому, что Leaflet пересоздаёт маркеры при каждом обновлении позиции и хука на это не даёт; устаревший класс с чужих маркеров снимается в том же проходе.

### Позиционирование карты по выбранной цели

Выбор цели (в таблице, по маркеру или скриптом) меняет per-user хелпер `input_text.ais_selected_mmsi_<slug>`. `src/js/ais-select-bridge.js` опрашивает его раз в 400 мс и на **изменение** значения: (1) центрирует карту на координатах цели через Leaflet-инстанс `ha-map` (`leafletMap` / `_leafletMap`), при этом зум **никогда не уменьшается**, а полностью отдалённая карта подтягивается до `FOCUS_MIN_ZOOM = 12`; (2) вешает на маркер класс `ais-blink` — CSS-анимация из четырёх плавных пульсаций (~2.6 с), после чего класс снимается. Если маркера ещё нет в DOM (карта не успела перерисоваться после панорамирования), делается одна повторная попытка через 400 мс. Опрос выбран вместо подписки потому, что объект `hass` подменяется целиком на каждое обновление состояния, а хука без собственной карточки нет.

### Анимация таблицы и авто-сворачивание при выборе

Полная таблица закрывает бо́льшую часть карты, поэтому цель, на которую карта только что отцентрировалась, оказывалась **под** таблицей. Теперь при смене выбора `src/js/ais-select-bridge.js` (`collapseTableAfterSelect()`) проверяет `input_boolean.ais_table_expanded_<slug>`: если он `on` — вешает на элемент `flex-table-card` класс `ais-table-collapsing` (схлопывание к правому верхнему углу, 220 мс) и **только после** окончания анимации вызывает `input_boolean.turn_off`. Порядок важен: conditional-карточка удаляется из DOM мгновенно, поэтому переключить хелпер сразу — значит не показать анимацию вовсе.

Сворачивание запускается **строго в момент, когда маркер цели начал мигать** (колбэк `onBlink` из `focusSelectedTarget()`) — последним шагом цепочки «выбор → центрирование → мигание → минимизация», и никогда на простое переключение тумблера. Первый проход опроса только **запоминает** текущее значение хелпера (`selectionPrimed`): выбор переживает перезагрузку и смену view, и без этого он выглядел бы как свежий, схлопывая таблицу сразу после её открытия. Если маркер цели на карте так и не нашёлся (цель не отрисована), колбэк всё равно срабатывает после единственной повторной попытки: иначе выбор такой цели молча оставлял таблицу раскрытой.

Класс сворачивания **снимается сразу после анимации**, а каждый опрос дополнительно зачищает залипшие метки (`clearCollapseMarks()`): Home Assistant не всегда уничтожает conditional-карточку и часто переиспользует тот же элемент `flex-table-card`, и оставшийся класс с `animation-fill-mode: forwards` делал таблицу невидимой при повторном включении тумблера.

Обратное направление (появление) анимировать из JS нельзя по той же причине, поэтому unfold-анимация (`@keyframes ais-table-unfold`, те же 220 мс) описана в `card_mod` обеих таблиц в `01_ais_map.yaml` — она проигрывается при любом переключении тумблера. Сам тайл-тумблер даёт тактильный отклик на нажатие (`ha-card:active { transform: scale(0.97) }`).

### Карточка деталей выбранной цели

**Почему она нужна.** У домена `geo_location` в Home Assistant **нет** собственного more-info-контрола: `more-info-content` подставляет `more-info-default`, который в актуальном фронтенде рендерит `nothing`. Поэтому клик по цели (и в таблице, и по маркеру на карте) показывал только состояние сущности (дистанцию) и график истории — все данные судна лежат в атрибутах, которых этот диалог не выводит вовсе.

**Решение.** Клик по любой ячейке таблицы больше не открывает пустой диалог (`clickable: false`), а **выбирает** цель: колоночный `tap_action` вызывает `input_text.set_value` для `input_text.ais_selected_mmsi`, подставляя MMSI через ссылку `col[N]` (индекс колонки MMSI в списке `columns`; в компактной таблице эта колонка добавлена с `hidden: true` — она нужна только как источник значения). Одинаковые действия во всех колонках держатся YAML-якорем (`&select_target_*` / `*select_target_*`) — `build.py` разворачивает якоря при сборке.

Отдельный `conditional`-оверлей в левом нижнем углу карты рендерит `markdown`-карточку со **всеми** полями цели `geo_location.ais_<mmsi>`: MMSI, имя, позывной, тип, длина/ширина, порт назначения, ETA, nav status, дистанция, SOG, COG, heading, rate of turn, координаты, время обновления; ниже — кнопка **Close**, очищающая хелпер. Условие показа проверяет `state_not: ""` **и** `state_not: "unknown"` (пустое значение — после закрытия, `unknown` — на свежей установке). Если цель ушла из эфира, карточка сообщает об этом вместо пустых полей.

**Хелпер `input_text.ais_selected_mmsi`** создаётся идемпотентно скриптом `helpers/provision_helpers.py` (шаг `provision_helpers` в `deploy.sh --install`) в `.storage/input_text`. Ключ `mode: text` **обязателен**: без него HA валит настройку всего компонента (`Error during setup of component input_text: 'mode'`) и хелпер не появляется. По той же причине провижининг сверяет **все** ключи записи и чинит её, а не только имя.

**Клик по маркеру на карте** тоже открывает эту карточку. Карта не даёт `tap_action` для маркеров — она всегда бросает событие `hass-more-info` (`bubbles: true, composed: true`). Собственный Lovelace-модуль `src/js/ais-select-bridge.js` слушает это событие на `window` в **capture-фазе** (то есть раньше обработчика `<home-assistant>`), и если `entityId` начинается с `geo_location.ais_`, отменяет событие и вместо пустого диалога пишет MMSI в `input_text.ais_selected_mmsi`. Все прочие сущности не затрагиваются. Модуль деплоится в `/config/www/ais-select-bridge.js` и регистрируется как ресурс шагом `deploy_card_deps()` в `deploy.sh`; версия в `?v=` — короткий хеш содержимого, поэтому правка подхватывается без ручной очистки кэша браузера.

**Габариты карточки деталей.** Оверлей прижат к левому нижнему углу контейнера карты (`left/bottom: 12px`, `top: 84px`), ширина задаётся CSS-длиной из конфига (`detail.width` → `${AIS_DETAIL_WIDTH}`) с ограничением `max-width: calc(100vw - 48px)`, а `max-height: 100%` + `overflow: auto` дают скролл вместо обрезания. Кнопка закрытия — компактная карточка `tile` (`button` рендерит огромный блок с иконкой). Скругления у карты, обеих таблиц и карточки деталей берутся из `var(--ha-card-border-radius, 12px)`, а обёртка `custom:mod-card` сделана прозрачной (`background/border/box-shadow: none`), чтобы не рисовать вторую рамку вокруг карты.

### Скролл таблиц и отступы ячеек

Скругления и скролл таблицы задаются **через собственный `css:` карточки**, а не через card-mod: `flex-table-card` сам строит свой shadow DOM и вешает `overflow: auto` на **хост-элемент**, поэтому правила card-mod для `ha-card`/`ha-card > div` не давали эффекта (углы оставались прямыми, отступов не было). Ключ `:host` (именно `:host`, а не `.type-custom-flex-table-card` — классовый селектор внутри shadow root не может совпасть с собственным хостом) даёт `border-radius: var(--ha-card-border-radius, 12px)` + `overflow: hidden`, а `ha-card` внутри — `overflow: auto` по обеим осям. Скролл `tbody` (`display: block`) не используется: при горизонтальном скролле вертикальный ползунок уезжал за пределы видимости. Заголовок держится через `thead th { position: sticky; top: 0 }`. Собственный `padding: 16px` таблицы снят, отступы задаются ячейками (`padding: 8px 10px`, у первой/последней колонки дополнительно `padding-left/right: 16px`). В карточке деталей скроллится **только markdown-часть**; кнопка Close вынесена из скролл-контейнера и прижата к низу (`justify-content: flex-end`), поэтому всегда видна и не выходит за нижний край карты. Порядок полей в карточке деталей совпадает с порядком колонок полной таблицы.

### Значки по типу цели

Таблица `_TYPE_MARKS` в `geo_location.py` сопоставляет декодированный `SHIP_TYPE` (лукап библиотеки `nmea2000`) с парой «unicode-глиф + mdi-иконка»: `⛵` sailing, `🛥` pleasure, `🛳` passenger, `🚢` cargo (и по умолчанию), `🛢` tanker, `🚤` tug/port tender, `🎣` fishing, `⚡` high speed craft, `🧭` pilot, `🚁` SAR, `🚑` medical, `🚓` law enforcement и т.д. Совпадение идёт по **префиксу** значения, поэтому все варианты `(hazard cat X/Y/Z/OS)` покрываются одной записью.

Глиф уходит в подпись маркера (`map_label`), потому что `ha-entity-marker` рендерит подпись обычным текстом; mdi-иконка ставится в `_attr_icon` и используется в more-info и списках сущностей. Режим `label_mode: icon` намеренно не применяется — он рисует только иконку и полностью убирает скорость/длину с карты.

### Навигационный статус, когда его нет в эфире

Class B position report **не содержит** поля `navStatus` (у Class A оно может быть `unavailable`), поэтому колонка `Nav Status` была пустой у большинства целей. `AisTarget._nav_status()` подставляет статус из скорости, используя формулировки AIS дословно:

- SOG `< 0.2` узла → `Moored`;
- SOG `> 0` → `Under way sailing` для парусного судна, иначе `Under way using engine`;
- скорости нет вовсе → `—` (ничего не выдумываем).

Шрифт подписи уменьшен через CSS-переменные `ha-map` (`--ha-marker-size`, `--ha-marker-font-size`, плейсхолдеры `${AIS_MARKER_SIZE}` / `${AIS_MARKER_FONT_SIZE}`, опции `map.marker_size` / `map.marker_font_size`). Отдельный размер шрифта именно для скорости и длины технически невозможен: `ha-entity-marker` вставляет подпись как **обычный текст** (HTML экранируется), поэтому уменьшается шрифт всей подписи целиком.

### Ширина колонки с именем судна

Таблица использует `table-layout: fixed` (это необходимо, чтобы колонки шапки и тела оставались выровненными, пока тело скроллится). Побочный эффект: ширина делится **равномерно** между колонками, поэтому длинные имена судов обрезались.

Колонке имени задана собственная ширина через `thead th:first-child, tbody td:first-child` — значение берётся из `table.vessel_column_width` (плейсхолдер `${AIS_VESSEL_COL_WIDTH}`, по умолчанию `220px`). Указывается и `width`, и `min-width`: при фиксированной раскладке один `min-width` игнорируется.

### Расстояние до цели: отсчёт от нашей лодки

`AisTarget._distance_km()` считает расстояние от **живой позиции нашей лодки**, а не от домашних координат Home Assistant (на движущейся лодке это бессмысленная точка отсчёта — именно поэтому колонка `Dist (km)` показывала непонятные значения).

Приоритет источника точки отсчёта (`AisTargetsManager.own_position()`):
1. **Собственный GNSS-фикс прямо с шины** — PGN 129029 (GNSS Position Data), при его отсутствии PGN 129025 (Position Rapid Update). Это точные координаты приёмников с антеннами под открытым небом, декодируются тем же `AisBusClient` (поле `own_position`).
2. AIS-цель с MMSI из `own_mmsi` — фолбэк (собственное AIS-сообщение).
3. Иначе расстояние **не показывается вообще**.

`device_tracker.nevera` в качестве источника **не используется**: это HA-шаблон, который получает тот же самый фикс через интеграцию `nmea2000` плюс удержание последней позиции, и на prod он публиковал частичный фикс (`longitude 0.0`), из-за чего все цели оказывались в ~1500 км. Домашние координаты Home Assistant тоже не используются — на движущейся лодке это бессмысленная точка отсчёта.

### Радиус карточек и геометрия оверлеев (проверено на живом стенде)

Проверка фактического DOM дашборда в браузере (headless Chrome на локальном stage) дала две конкретные причины давних жалоб «нет скруглений» и «кнопка Close уезжает за карту»:

1. **Радиус.** Тема инстанса задаёт `--ha-card-border-radius: 0`, поэтому запись `var(--ha-card-border-radius, 12px)` НИКОГДА не падала на 12px — вычисленный `border-radius` у всех карточек был `0px`. Теперь радиус задаётся literal-значением через плейсхолдер `${AIS_CARD_RADIUS}` (опция `style.card_radius`, по умолчанию `12px`) — карта, обе таблицы и карточка деталей скруглены независимо от темы.
2. **Высота оверлеев.** Контейнер `custom:mod-card` на ~24px выше самой карты: свёрнутые (нулевой высоты) обёртки `hui-card` остальных оверлеев всё равно добавляют зазоры `vertical-stack`. Поэтому `bottom: 12px` уводил низ карточки деталей ниже карты и кнопка Close выходила за её край. Теперь высота оверлеев считается от высоты КАРТЫ: `height: calc(${AIS_MAP_HEIGHT} - 96px)` (84px сверху + 12px снизу), а markdown-часть деталей ограничена `calc(${AIS_MAP_HEIGHT} - 180px)`.

Отступы ячеек (`padding: 8px 10px`, у крайних колонок `16px`) применяются штатно — это подтверждено вычисленными стилями `td`/`th` в реальном DOM.

### Отступы в компактной таблице и скругления кнопки Close (проверено в DOM)

- **«Паддинги не работают».** Правила `padding: 8px 10px` / краевые `16px` применялись корректно (подтверждено вычисленными стилями `td`/`th`), но в свёрнутом оверлее колонка имени имела `min-width: ${AIS_VESSEL_COL_WIDTH}` (220px), из-за чего таблица становилась шире карточки и правый отступ уезжал за пределы видимой области. `min-width` оставлен только в полной таблице; в компактной он убран — таблица укладывается в ширину карточки (`scrollWidth == clientWidth`), отступы видны с обеих сторон.
- **Кнопка Close без нижних скруглений.** Прозрачная обёртка `ha-card` оверлея деталей имела `overflow: hidden` при радиусе темы `0`, поэтому срезала скруглённые нижние углы плитки Close, прижатой к её низу. Обёртке задан `overflow: visible` (скроллится только markdown-часть, поэтому клиппинг там не нужен).

### Состояние дашборда — на сервере, но привязанное к пользователю

Home Assistant **не имеет** пер-юзерных состояний сущностей: обычный хелпер глобален, поэтому выбор судна и развёрнутость таблицы одного пользователя видели все остальные (состояние меняется на сервере и рассылается по WebSocket всем клиентам).

Решение — состояние по-прежнему **хранится на сервере** (переживает перезагрузку браузера и синхронизируется между устройствами одного пользователя), но в **отдельной паре хелперов на каждого пользователя**:

- `input_text.ais_selected_mmsi_<slug>` — выбранная цель;
- `input_boolean.ais_table_expanded_<slug>` — свёрнут/развёрнут оверлей таблицы.

`<slug>` — HA-слаг имени пользователя, либо `u_<первые 8 символов user id>`, если в имени нет ASCII-букв/цифр (например, кириллическое имя). Хелперы создаются идемпотентно `helpers/provision_helpers.py --auth <.storage/auth>` (шаг `provision_helpers` в `deploy.sh --install`) по списку активных не-системных пользователей. Хелперы без суффикса тоже остаются — как фолбэк, когда пользователь не определён.

**Почему нужен `custom:ais-user-scope`.** Lovelace не позволяет подставить entity_id по текущему пользователю: `conditional`-карточка и `tap_action` у `flex-table-card` требуют литеральный entity_id и не шаблонизируются. Поэтому в шаблоне `01_ais_map.yaml` пишется токен `__AIS_USER__` (например `input_text.ais_selected_mmsi__AIS_USER__`), а карточка-обёртка `custom:ais-user-scope` (в составе `src/js/ais-select-bridge.js`) рекурсивно заменяет его на суффикс **текущего** пользователя и создаёт дочернюю карточку через `window.loadCardHelpers()`. Обёртке задан `display: contents`, чтобы она не создавала собственный бокс — иначе она участвовала бы во flex-колонке `vertical-stack` и абсолютно спозиционированные оверлеи резервировали бы высоту.

`ais-select-bridge.js` (перехват клика по маркеру карты) пишет MMSI в хелпер того же пользователя — слаг вычисляется той же формулой, что и в `provision_helpers.py`.

**При добавлении нового пользователя HA** нужно один раз прогнать `ha/ais/deploy.sh --stage|--prod --install`, чтобы ему завели его пару хелперов (до этого он будет работать с общим фолбэком).
