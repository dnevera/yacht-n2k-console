...
Сворачивание запускается **строго в момент, когда маркер цели начал мигать** (колбэк `onBlink` из `focusSelectedTarget()`) — последним шагом цепочки «выбор → центрирование → мигание → минимизация», и никогда на простое переключение тумблера. Первый проход опроса только **запоминает** текущее значение хелпера (`selectionPrimed`): выбор переживает перезагрузку и смену view, и без этого он выглядел бы как свежий, схлопывая таблицу сразу после её открытия.
