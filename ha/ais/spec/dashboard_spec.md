...
Единственный скролл-контейнер таблицы — **контентный `div` внутри `ha-card`** (`ha-card > div { overflow: auto }` по обеим осям), а сама `ha-card` получает `overflow: hidden`, `border-radius: var(--ha-card-border-radius, 12px)` и `display: flex; flex-direction: column`. Скролл на самой `ha-card` (как было раньше) не давал скруглениям работать: таблица закрашивала углы, и карточка выглядела прямоугольной. Прежняя схема со скроллом `tbody` (`display: block`) ломалась при горизонтальном скролле: вертикальный ползунок оказывался далеко справа за пределами видимой области. Заголовок держится на месте через `thead th { position: sticky; top: 0 }` с фоном `var(--card-background-color)`. Собственный `padding: 16px` таблицы снят (при горизонтальном скролле он давал неравномерный отступ) — отступы задаются ячейками: `padding: 8px 10px`, у первой/последней колонки дополнительно `padding-left/right: 16px`. `table-layout: fixed` не используется: ширина колонки имени задаётся `min-width` (`table.vessel_column_width`), а общая минимальная ширина — `table.min_width_px`.
