...
Атрибут `map_label` формирует интеграция (`AisTarget._map_label()`) в виде `XX 6.4kn 120m`, где `XX` — **двухбуквенный бейдж** из имени судна (`AisTarget._initials()`, как в дефолтном поведении HA). Полное имя на карту намеренно не выводится (маркер превращался в «кляксу»), а недоступные поля просто опускаются — ни `unknown`, ни прочерков на карте быть не должно. Beam в подпись не входит, только длина.
