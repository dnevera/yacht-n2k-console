...
- **Fixed: pinch zoom and the +/-/reset buttons on a phone, for real this time - and tooltips now need a long press.** Reading the card bundle showed the actual cause, which entries (23)/(24) had guessed wrong: **the card already ships its own touch controller** (`disable_pinch_to_zoom`), and it zooms by dispatching a *synthetic* `wheel` event at the drag layer. Since entry (20) we set `scrollZoom: false`, so that wheel did nothing - pinch was dead by construction, and the hand-written pinch of entry (23) was fighting the card's controller for the same axis.
  - `helpers/build.py::apply_zoom_controls()`: `scrollZoom` back to `True`. A real mouse/trackpad still must not zoom, so `plotly_touch_patch_shapes.js` swallows every `isTrusted` wheel event before Plotly sees it (without `preventDefault()`, so the page still scrolls over a chart); the synthetic, pinch-derived ones pass, and are blocked only when they would cross `zoom_min_hours`/`zoom_max_hours`.
  - `layout.transition` dropped: the eased 300 ms replot looked fine on a button press but turned every pinch frame into a rubber-banding redraw.
  - **Buttons.** The same card controller listens for touches in the capture phase *on the plot div*, and treats a second tap within 250 ms as the start of its double-tap zoom, `stopPropagation()`-ing it - including taps on the modebar, which is a child of that div. So a burst of +/- taps was eaten by the card itself. `plotly_touch_patch_shapes.js` now puts its own capture listener on the **parent** element, where it runs first, and stops modebar touches from reaching the controller at all. All the machinery of entry (24) (synthetic `btn.click()`, `gd.__zoomWindow`, debounced re-fetch) is gone; the browser's own click presses the buttons and `plotly_zoom_step_buttons.js` is back to its plain, direct path.
  - **Tooltips.** Plotly pops a tooltip up for any tap; it is meant to be the reward for a deliberate long press, so a short tap now explicitly takes it back down (`mouseout`/`mouseleave` on the drag layer) while the 400 ms hold keeps it.
- Tests: the pinch/modebar-tap suites in `tests/js/wind_chart_snippets.test.js` were replaced by a wheel-gate suite (trusted wheel blocked, synthetic one allowed, limits enforced) and a tooltip/modebar suite; `tests/test_sailing_dash.py` expects `scrollZoom: True` and no `layout.transition`.

## 2026-08-14 (24)

- **Fixed: on a phone both the +/-/reset buttons and the pinch of entry (23) were still glitchy.** Two causes, both fixed in `src/js/common/plotly_touch_patch_shapes.js` (+ `src/js/common/plotly_zoom_step_buttons.js`):
  - **The tap itself was unreliable.** Skipping the modebar in the touch handlers (entry 23) was not enough: the browser still only synthesises a `click` after its own delay and drops it on the slightest finger movement, and Plotly's buttons listen for exactly that `click`. The snippet now tracks the `.modebar-btn` the finger landed on and calls `btn.click()` itself from `touchend` (cancelling the synthesised one), so a tap fires immediately and survives a small slide.
  - **Every tap/frame raced a network round-trip.** Both the buttons and the pinch called `plot({should_fetch: true})`, which re-fetches the whole history; while that was in flight the next tap read a stale `layout.xaxis.range`, so steps were lost and the window snapped back. All zooming now goes through one entry point per chart (`gd.__zoomWindow`, installed by the touch snippet, used by the buttons when present): it repaints instantly from the data already in memory (`should_fetch: false`) against the window the *previous* tap asked for, and debounces the single re-fetch by 250 ms. Pinch frames additionally skip while a replot is still running, and a two-finger `touchstart` now takes the gesture away from Plotly's own pan (`stopPropagation`/`preventDefault`) so the two do not fight over the axis.
- Tests: `tests/js/wind_chart_snippets.test.js` gained a modebar-tap suite (tap clicks the button, a slide off it does not, a burst of taps composes and produces exactly one re-fetch).

## 2026-08-14 (23)

- **Added: two-finger pinch zoom on the charts, and fixed the +/-/reset buttons being unreliable on a phone.** Since entry (20) the wheel — and with it Plotly's own pinch handling — is off (`scrollZoom: false`), which left the modebar buttons as the only way to zoom; on a touch screen that is not what anyone expects, and those buttons themselves misbehaved:
  - **Pinch zoom** is now implemented by `src/js/common/plotly_touch_patch_shapes.js` itself (`touchstart`/`touchmove`/`touchend` with two touches), driving the exact same path the +/- buttons use: it mutates the plot div's `layout.xaxis.range` around the window's centre and calls the card's public `enterBrowsingMode()`/`plot()`. Frames during the gesture are throttled (~60 ms) and replot from the data already in memory (`should_fetch: false`); only lifting the fingers fetches the history of the new window. The configured `zoom_min_hours`/`zoom_max_hours` clamp the gesture exactly like they gate the buttons.
  - **Fixed: a tap on a zoom button often did nothing (and buzzed instead).** The long-press-for-tooltip patch is bound to the whole chart div in the capture phase, and the modebar is a child of it — a finger resting on a button for more than `HOLD_MS` (400 ms, entirely normal on a phone) turned the tap into a hover gesture, aborting the click. Both touch handlers now bail out on `e.target.closest('.modebar')`.
  - **Fixed: the first tap on the chart was always wasted.** `apply_zoom_controls()` set `displayModeBar: True`, and Plotly only fades that modebar in on hover — on a touch screen the first tap produced the "hover" and only the second one pressed a button. It is now `'always'`.
- Tests: `tests/js/wind_chart_snippets.test.js` gained a pinch-gesture suite (zoom in on spreading fingers, window stays centred, no re-fetch mid-gesture, one fetch on release, clamping at `zoom_min_hours`); `tests/test_sailing_dash.py::test_zoom_controls_unlock_only_the_time_axis` updated for `displayModeBar: 'always'`.

## 2026-08-13 (22)

- **Changed: zoom step per click is smaller, the +/- buttons now enforce their limit from the very first load, and the reset button no longer jumps to a stale scale.** Three separate long-standing complaints, fixed together since they all live in the same modebar:
  - **Smaller zoom step.** Plotly hardcodes zoomIn2d/zoomOut2d at x0.5/x2 per click with no config knob for it. New `src/js/common/plotly_zoom_step_buttons.js` (wired via `config.modeBarButtons` in `apply_zoom_controls()`) replaces all three native buttons with custom ones (icons copied verbatim from the bundled Plotly.js, since this bundle does not expose `Plotly` as a global) that use a gentler x0.8/x1.25 step. Zoom in/out work by mutating the plot div's own `layout.xaxis.range` — exactly what the card's own `getVisibleRange()` reads back — then calling the card's public `enterBrowsingMode()`/`plot()`, the same path its native zoom/pan already goes through; no `Plotly.relayout` needed.
  - **No resize/snap-back any more, only a limit check.** `plotly_zoom_scale_limit.js` (which used to stretch/shrink an out-of-bounds window back to `zoom_min_hours`/`zoom_max_hours`) is removed entirely — `apply_zoom_controls()` no longer sets `layout.xaxis.range` at all. The custom zoom buttons above simply do nothing once the window is already at/past its limit; the existing `patchZoomButtons()` grey-out (added in entry 21) is what actually stops the user.
  - **The "-"/"+" buttons now check the limit on the very first render too**, not only after the first gesture: `patchZoomButtons()` in `plotly_touch_patch_shapes.js` now also listens to `plotly_afterplot` (which fires on the initial draw as well), instead of only `plotly_relayout`/`plotly_relayouting`.
  - **Reset button now matches the double-click reset.** Plotly's native `resetScale2d` restores the range it captured once at mount time, which does not track "now" — after the dashboard had been open a while, clicking it visibly jumped to a stale scale instead of doing a proper reset. The new custom reset button instead clicks the very same hidden `button#reset` the card's own `on_dblclick` handler already uses (`plotly_reset_on_dblclick.js`), so both now behave identically.
- Tests: `tests/js/wind_chart_snippets.test.js` gained a full suite for `plotly_zoom_step_buttons.js` (step factors, no-op at the configured limit, reset delegating to the hidden button); `tests/test_sailing_dash.py` updated for `modeBarButtons` now being a `$fn` include and for the dropped `layout.xaxis.range` override.

## 2026-08-13 (21)

- **Added: the modebar's `+`/`-` zoom buttons now grey out and stop accepting clicks once the configured `forecast_min_scale`/`forecast_max_scale` limit is actually reached** (visual/click feedback for the limits added in entry (18); those limits already worked, but nothing showed the user why a further click did nothing).
- Extended the existing per-chart `src/js/common/plotly_touch_patch_shapes.js` patch with `patchZoomButtons()`: it reads `zoom_min_hours`/`zoom_max_hours` off the card's own config (same values `apply_zoom_controls()` already injects for `plotly_zoom_scale_limit`), listens to Plotly's own `plotly_relayout`/`plotly_relayouting` events on the chart div (no polling, no DOM scanning of its own) and toggles opacity/`pointer-events` on the `[data-title="Zoom in"]`/`[data-title="Zoom out"]` modebar buttons based on the current window width. When neither limit is configured the patch does not attach at all.
- Tests: `tests/js/wind_chart_snippets.test.js` gained cases for both directions (button disabled at its limit, re-enabled once the window moves away) and for the no-limits-configured no-op case.

## 2026-08-13 (20)

- **Changed: mouse wheel/trackpad/pinch zoom removed entirely from both charts — zoom now works ONLY through the modebar's +/-/reset buttons, which get their animated transition back.** The wheel kept behaving unpredictably across the last several fixes (momentum tail after the gesture, a diagonal tick sneaking in a pan) because the browser hands over a raw `wheel` event that mixes OS-level momentum and multi-axis deltas indistinguishably from real input — there is no reliable way to sanitize it further. Removing it as a zoom trigger altogether is simpler and fully deterministic; panning by drag is completely unaffected.
- `helpers/build.py::apply_zoom_controls()`: `scrollZoom` is now always `False` (both enabled and disabled branches); `layout.transition = {"duration": 300, "easing": "cubic-in-out"}` is set whenever zoom is enabled (and removed when it is disabled), so a modebar button press now eases into the new range instead of snapping.
- `src/js/common/plotly_touch_patch_shapes.js`: removed the `patchWheel()` helper added in the previous entry (deltaX stripping / momentum-burst cutoff) — with the wheel no longer wired to zoom at all, there is nothing left for it to sanitize. The long-press-for-tooltip touch patch is untouched.
- Tests: replaced the `patchWheel` JS cases in `tests/js/wind_chart_snippets.test.js` with a check that no wheel-gesture patch is installed any more (the long-press touch patch still is); `tests/test_sailing_dash.py::test_zoom_controls_unlock_only_the_time_axis` and `test_forecast_scale_limits_reach_both_charts_and_do_not_affect_panning` updated for `scrollZoom: False` and the new `layout.transition`.

## 2026-08-13 (19)

- **Fixed: zoom kept "coasting" after the wheel/trackpad gesture ended, and a diagonal wheel tick could pan the chart WHILE it zoomed.** Neither is an animation setting: macOS/trackpad OS-level momentum scrolling keeps sending real `wheel` events with decaying `deltaY` for a while after the user's fingers leave the pad, and `scrollZoom` faithfully keeps zooming on every one of them; a diagonal two-finger swipe on the same gesture also carries a non-zero `deltaX`, which `scrollZoom` can fold into a pan at the same time as the zoom.
- Added `patchWheel()` to the existing per-chart `src/js/common/plotly_touch_patch_shapes.js` snippet (same pattern as the existing touch-gesture patch - one listener bound directly to each chart via its own DOM element, no `MutationObserver`/`setInterval`, nothing global): every `wheel` event has `deltaX` stripped before it reaches Plotly (zoom can never carry a pan component), and a continuous run of wheel events longer than ~600 ms is treated as the OS momentum tail and dropped, stopping the zoom exactly where the user's own gesture left it. Existing zoom/pan controls, buttons and dragmode are untouched.
- Tests: `tests/js/wind_chart_snippets.test.js` gained a `WheelEvent`/`EventTarget` based regression (diagonal tick never lets `deltaX` through, a short deliberate burst is not suppressed, a burst dragging on past ~600 ms is dropped).

## 2026-08-13 (18)

- **Added: `forecast_min_scale`/`forecast_max_scale` — limits on how far the user can ZOOM on the time axis (mouse wheel, pinch, the +/- modebar buttons), without touching panning at all.** After several failed prior attempts that tried to bound the outer edges of the window (`minallowed`/`maxallowed`) or otherwise interfered with the gesture itself, this iteration limits only the window's WIDTH: `forecast_min_scale` (hours) is the narrowest span zoom-in may reach, `forecast_max_scale` (days) the widest span zoom-out may reach; either can be omitted/`0` to leave that direction unlimited.
- New shared snippet `src/js/common/plotly_zoom_scale_limit.js` on `layout.xaxis.range`, reading the card's own `visible_range` (`getFromConfig('visible_range')`, `[msNumber, msNumber]` — set by `plotly-graph` itself while browsing) and returning `undefined` for any window whose width is already within bounds. The card's own layout merge (`merge({}, layout, {xaxis:{range: visible_range}}, ..., layout)`) then leaves `visible_range` completely untouched for `undefined` — so a pure pan (which never changes the width) and any in-bounds zoom are never fought by this snippet; only a window that is already too narrow/too wide is stretched/shrunk back around its own centre.
- `helpers/build.py`: new `resolve_zoom_scale_limits()` (hours/days -> hours, invalid/non-positive disables that bound only) and `apply_zoom_controls()` now also injects `zoom_min_hours`/`zoom_max_hours` and wires the snippet into both the wind and the wave chart identically (no duplicated logic between the two). Documented in `config.yaml.template`, added to the `--config` wizard (`helpers/configure.py`, `--forecast-min-scale`/`--forecast-max-scale`).
- Verified live against the deployed stage in a headless browser: a real drag-pan gesture keeps the exact window width and moves freely with no bound in either direction (tested past both the configured history and forecast windows); repeated mouse-wheel zoom-in stops exactly at the configured `forecast_min_scale` (1 h) and zoom-out stops exactly at `forecast_max_scale` (168 h = 7 d) - confirmed by reading the card's live `contentEl.layout.xaxis.range` after each gesture, not just the built YAML.
- Tests: 340 Python tests pass (3 skipped), all JS assertions pass; new `test_resolve_zoom_scale_limits` and `test_forecast_scale_limits_reach_both_charts_and_do_not_affect_panning` cover the hours/days conversion, both charts receiving the same limits, and the option being fully absent (no override at all) when not configured.

## 2026-08-12 (13)

- **Fixed: "the NMEA sensors do not sync on prod" — auto-discovery was binding the DEAD twin of every instrument.** Reinstalling the `nmea2000` integration leaves the previous entity ids in the registry, so the vessel Pi5 held two entities per PGN: `wind_data_pk_f9e756…_wind_speed` (`unavailable` since the reinstall) next to `wind_data_raymarine_20_442559_pk_a00872…_wind_speed` (4.8 kt). `match_entities()` looked at entity ids only and picked the first match — the dead one — so every `sensor.boat_*` stayed `unavailable` while the bus carried ~1500 msg/min. `--force` could not help: the bytes it re-uploaded were the wrong mapping, not a stale one.
- `map_nmea_sensors.py` now reads the **live states of the target** (`fetch_live_states()` → `/api/states`) and orders candidates live-first/freshest-first (`prioritize_live()`), and numeric values additionally require a numeric state — without that STW bound to `..._speed_water_referenced_type` ("Paddle wheel"), the text enum sitting next to the speed.
- **Fixed a self-reference:** on a target where PGN 127250 yielded no entities the matcher mapped `heading` → `sensor.boat_heading_magnetic` and `variation` → `sensor.boat_magnetic_variation` — our OWN generated template sensors reading themselves, which can never become a number. `OWN_ENTITY_PREFIXES` now excludes them from the candidate list.
- **`deploy_sensors.sh` no longer swallows discovery failures.** The `|| true` after `map_nmea_sensors.py` meant a broken or empty mapping silently shipped the previous binding and the deploy reported success; discovery now runs with `--strict` (every required bus value must resolve to a LIVE entity) and the deploy aborts with an explanation instead.
- **`src/yaml/sensors/derived_n2k.yaml` is no longer tracked by git.** It is generated per target from one instance's PGN hashes, so it is not source: `build.py::ensure_derived_n2k()` regenerates it from the generic fallbacks when missing, keeping a fresh clone buildable.
- Cleaned the prod registry (Bug 3 of the skill): 27 dead `nmea2000` entities, 8 orphaned devices and 723 `deleted_entities` removed with backups of both registries. Verified live afterwards: `boat_wind_speed 8.3`, `boat_depth 4.5`, `boat_cog 8`, position 42.4345/18.6032. `boat_heading*`/`boat_magnetic_variation` were still `unavailable` at that point — and NOT because the compass is missing from the bus: a raw 60 s capture off `127.0.0.1:4001` carries PGN 127250 and 127258 (my earlier "physically not on the bus" claim was wrong). The real cause was the WHITELIST in the `nmea2000` config entry on prod — `pgn_include: 127505, 127508, 127506, 127488, 127489, 130306, 128267, 128259, 129025, 129026, 130310, 130312` lists neither of them, so HA never created the `vessel_heading_*`/`magnetic_variation_*` entities (they existed only in `deleted_entities`). Fixed on the target by widening `pgn_include` to the 44 PGNs actually present in the traffic.

## 2026-08-12 (12)

- **Fixed: the "Gusts next 1h" tile stayed `Unknown` for models that publish no gusts.** Two separate causes, both confirmed on the live stage: `ECMWF IFS 0.25°` really does return `windgusts_10m` as an array of `null`s (the key is present, the values are not), and the dash introduced in the previous entry was rejected by Home Assistant itself — `sensor.wind_gust_next_hour` carries `unit_of_measurement: kts`, i.e. it is a numeric sensor, so `Received invalid sensor state: — for entity sensor.wind_gust_next_hour, expected a number` was logged on every poll and the state fell back to `unknown` anyway.
- The sensor therefore stays numeric and reports the special `unknown` state (no more error per poll), and the **dash is rendered by the glance tile** in `src/yaml/dashboard/sections/04_wind.yaml`: `card_mod` hides the state text and draws `—` via `::after` when the state is `unknown`/`unavailable`. The value is legitimately absent for the selected model, not broken — and the sensor remains usable for the chart and for statistics.

## 2026-08-12 (11)

- **Verified against the live bus that the true-wind conversion itself is correct, and fixed what actually made the measured direction look wrong.** Six live samples off the real gateway (stage reads `bumblebee.local:4001`) gave a computed TWD of 176…256° against a forecast of 207° for the current hour — the mean matches, the spread does not: a raw NMEA series swings by tens of degrees between consecutive samples while the forecast is an hourly mean, so the measured arrows read as disagreeing with it. (The earlier "180° off" diagnosis was a comparison mistake of mine: it compared against `forecast_dir[0]`, the 00:00 sample, instead of the current hour.)
- **Added: measured series are averaged over the grid step of the ACTIVE Open-Meteo model** (`measured_averaging: model_step`, new global option; `none` restores raw samples). The window is not hard-coded — it is derived from the spacing of the forecast timestamps, so switching the model on the dashboard changes the averaging window with it. The new shared snippet `src/js/common/plotly_measured_average.js` replaces the fixed `resample: 30m` of both measured wind series and averages **directions as vectors** via sin/cos: an arithmetic mean of 350° and 10° yields 180°, the exact opposite of the correct 0°, and such a sample really is present in the recorder history.
- **Fixed: a model without gusts took the WHOLE flattened wind forecast to `unavailable`.** `state_attr(...)['windgusts_10m']` raised inside the template for models that do not publish gusts, killing speed and direction along with them; all three attributes now use `.get(key, [])`, and the "Gusts next 1h" tile shows a plain `—` instead of `Unavailable` — an absent value is not a broken one.
- **Fixed: the model dropdown listed raw API ids.** The selector now offers human readable titles (`ECMWF IFS 0.25°`, `NOAA GFS`, `DWD GWAM (global)`, …). Since the helper stores exactly what it shows, `build.py` injects the same `title -> id` table into both REST `resource_template`s (token `MODEL_IDS_WIND`/`MODEL_IDS_WAVE`), so there is a single source of truth for the options and for the URL lookup; `Best match (auto)` and any unknown/unavailable state still emit no `models=` parameter at all.

## 2026-08-12 (10)

- **Fixed: the measured wind direction did not match the open-meteo forecast — the two were in different frames of reference.** `sensor.wind_direction_history` (and `boat_wind_angle`) charted the raw `..._wind_angle` of PGN 130306, which is the angle relative to the **bow** (0° = wind straight onto the bow), while open-meteo's `wind_direction_10m` is the geographic direction the wind blows **from**. Every measured arrow was therefore rotated by the boat's heading — heading south that looks exactly like a swapped N/S. Two new generated virtual sensors do the conversion from the sensor data: `sensor.boat_true_wind_direction` and `sensor.boat_true_wind_speed`. The correction follows the PGN's own `reference` field, now auto-discovered by `helpers/map_nmea_sensors.py` (exact `_reference` + `wind` match, so it is never confused with `..._cog_reference` of PGN 129026): *true north referenced* is taken as is, *true boat referenced* only gets the heading added, and *apparent* (also the fallback when the field is missing, as on a masthead unit) gets the full vector conversion `flow = -AWS·(sin θ, cos θ) + SOG·(sin COG, cos COG)`, `θ = HDG + AWA` — a moving boat shifts the apparent **direction**, not just the speed. The chart's direction series follows the true direction, so measured and forecast arrows finally share one reference.
- **Changed: the "no source must become 0" regression only guards sources read with a numeric default.** The wind `reference` is a textual enum whose absence legitimately means "assume apparent", so requiring it in `availability` would have made the whole alias unavailable on buses that omit it.
- **Fixed: two layout assertions hard-coded a `heading` card as the first card of a chart section.** A section may carry its title on the model selector instead (as the Wind section now does), so the tests locate the section by its selector/chart and only check the shared-row widths when a heading card is actually present.
...
- **Fixed: ApexCharts wind chart tooltip was missing measured gusts and had a different row order than Plotly:**
  - `04_wind_apexcharts.yaml` had no "Gusts (measured)" series at all — the shared tooltip could only ever show Measured/Forecast/Gusts(forecast), so the user never saw a measured-gust value next to the measured speed at a given time. Added it as a fourth series (`sensor.boat_wind_speed`, `group_by: {func: max, duration: 30m}`), the ApexCharts equivalent of the Plotly engine's `plotly_wind_gust_bucket` bucketing, and extended `stroke.width`/`stroke.curve` from 3 to 4 entries accordingly.
  - Reordered series/entities in both chart engines to the same row order — Measured, Gusts (measured), Forecast, Gusts (forecast) — so the single shared/unified tooltip reads identically regardless of `chart_engine`. Both engines already stopped measured traces exactly at "Now" (`extend_to`/`extend_to_present: false`), so the forecast-only region of the tooltip was already correct and untouched by this change.
