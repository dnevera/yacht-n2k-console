...
# Entities generated BY THIS SCRIPT. Reinstalling the nmea2000 integration on a
# target leaves the previous entity ids in the registry, and the aliases below
# match the very suffixes we look for (`..._heading_magnetic`, `..._variation`),
# so without this guard a target without PGN 127250 mapped `heading` onto
# `sensor.boat_heading_magnetic` — a template sensor pointing at itself, which
# can never become a number.
OWN_ENTITY_PREFIXES = (
    "sensor.boat_",
...
# Every key the dashboard really needs on the bus. `--strict` fails when one of
# them stays unmapped, so a deploy can never silently ship the previous binding.
REQUIRED_KEYS = ("stw", "depth", "wind_speed", "wind_angle", "cog", "sog", "latitude", "longitude")


def fetch_live_states(base_url, token):
    """entity_id -> (state, last_updated) straight from the target instance.

    The registry alone cannot tell a live entity from a leftover duplicate of the
    same PGN: on the vessel Pi5 both `wind_data_pk_f9e756…_wind_speed`
    (unavailable since the last integration reinstall) and
    `wind_data_raymarine_20_442559_pk_a00872…_wind_speed` (4.8 kt) exist, and the
    matcher used to bind the dead one, leaving every `sensor.boat_*` unavailable
    while the bus was carrying 1500 messages a minute.
    """
    ...
    try:
        ...
        print(f"[WARN] Cannot read live states from {url}: {e}")
        print("[WARN] Falling back to registry order — a stale duplicate entity may be picked.")
        return {}
    return {
        s["entity_id"]: (str(s.get("state", "")).lower(), s.get("last_updated") or "")
        for s in states
        if s.get("entity_id")
    }


def prioritize_live(entity_list, live_states):
    """Live entities first (freshest first), dead duplicates last.

    Ordering rather than filtering keeps the matcher working unchanged and still
    allows a mapping when the states endpoint is unreachable.
    """
    if not live_states:
    ...
    for eid in entity_list:
        state, updated = live_states.get(eid, ("", ""))
        (live if state not in DEAD_STATES else dead).append((updated, eid))
    live.sort(key=lambda item: item[0], reverse=True)
    return [eid for _, eid in live] + [eid for _, eid in dead]
...
def discover_from_registry_file(reg_path, live_states=None):
    ...
    return match_entities(
        [e.get("entity_id") for e in entities if e.get("entity_id") and not e.get("disabled_by")],
...
def discover_from_api(base_url, token):
    ...
    try:
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            return match_entities(entity_ids, live_states=live_states)
    except Exception as e:
        print(f"[WARN] Failed to query HA API at {url}: {e}")
        return {}


def match_entities(entity_list, live_states=None):
    discovered = {}

    entity_list = [
        eid
        for eid in entity_list
        if isinstance(eid, str) and not eid.startswith(OWN_ENTITY_PREFIXES)
    ]
    entity_list = prioritize_live(entity_list, live_states)

    def is_numeric(eid):
        """True unless the target reports a non-numeric state for this entity.

        PGN fields sit side by side as `..._speed_water_referenced` (kt) and
        `..._speed_water_referenced_type` ("Paddle wheel"), and the loose suffix
        match returns either of them; without this check STW was bound to the
        text enum.
        """
        if not live_states or eid not in live_states:
            return True
        state = live_states[eid][0]
        try:
            float(state)
            return True
    ...
    def find_match(suffixes, numeric=False):
        for eid in entity_list:
            if not isinstance(eid, str):
                continue
            if numeric and not is_numeric(eid):
                continue
            for suf in suffixes:
    ...
    stw = find_match(["speed_water_referenced", "stw"], numeric=True)
    if stw: discovered["stw"] = stw

    depth = find_match(["water_depth", "depth"], numeric=True)
    if depth: discovered["depth"] = depth

    wind_speed = find_match(["wind_speed"], numeric=True)
    if wind_speed: discovered["wind_speed"] = wind_speed

    wind_angle = find_match(["wind_angle"], numeric=True)
    ...
    cog = None
    for eid in entity_list:
        if isinstance(eid, str) and eid.endswith("_cog") and is_numeric(eid):
            cog = eid
            break
    ...
    sog = None
    for eid in entity_list:
        if isinstance(eid, str) and eid.endswith("_sog") and is_numeric(eid):
            sog = eid
            break
    if sog: discovered["sog"] = sog

    lat = find_match(["_latitude"], numeric=True)
    if lat: discovered["latitude"] = lat

    lon = find_match(["_longitude"], numeric=True)
    if lon: discovered["longitude"] = lon

    pressure = find_match(["_pressure", "barometer", "pressure"], numeric=True)
...
def main():
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Exit non-zero when a required bus value stays unmapped instead of writing default fallbacks",
    )
    ...
    # The registry lists leftover duplicates of the same PGN as equals; only the
    # live states of the target tell which of them still carries data.
    live_states = fetch_live_states(api_url, token)
    ...
    if reg_path:
        ...
        mappings = discover_from_registry_file(reg_path, live_states=live_states)
    ...
    for k, v in mappings.items():
        state = live_states.get(v, ("?", ""))[0] if live_states else "?"
        print(f"  - {k}: {v} (state: {state})")

    # A key bound to a dead entity is as broken as an unmapped one: that is exactly
    # how the vessel Pi5 ended up with a "successful" deploy and an empty dashboard.
    missing = [
        key
        for key in REQUIRED_KEYS
        if key not in mappings
        or (live_states and live_states.get(mappings[key], ("", ""))[0] in DEAD_STATES)
    ]
    if missing:
        print(f"[ERROR] No live entity found for: {', '.join(missing)}")
        if args.strict:
            print(
                "[ERROR] Refusing to write a mapping bound to nothing: the dashboard would look "
                "deployed while every sensor.boat_* stays unavailable."
            )
            return 1
