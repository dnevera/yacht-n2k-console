#!/usr/bin/env bash
# env_profile.sh — resolves a NAMED TARGET PROFILE from ha/sailing-dash/.env.
#
# This subproject is self-contained: it reads ONLY ha/sailing-dash/.env (see
# .env.template) and never the repo root .env / deploy.conf, which belong to the
# ydnu-02 manager. That is what keeps a production host from leaking into a stage
# deploy — architecture, not a patch.
#
...
# lib/ lives in ha/sailing-dash/helpers/lib/ — .env belongs to the subproject root.
...
env_profile_load() {
    ...
    if [[ -f "${ENV_PROFILE_FILE}" ]]; then
        ...
        echo "       Create it:  cp ${ENV_PROFILE_DIR}/.env.template ${ENV_PROFILE_FILE}" >&2
        return 1
    fi
