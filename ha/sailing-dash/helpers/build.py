...
def ensure_derived_n2k():
    """Make sure the host-specific alias module exists before compiling sensors.

    `src/yaml/sensors/derived_n2k.yaml` is generated by map_nmea_sensors.py from
    the entity ids of ONE Home Assistant instance, so it is not source code and is
    no longer tracked by git (it used to leak one vessel's PGN hashes into every
    checkout and show up in unrelated diffs). A fresh clone therefore regenerates
    it here from the generic DEFAULT_FALLBACKS, keeping `build.py` runnable
    stand-alone; a deploy overwrites it with the real mapping of its target.
    """
    path = os.path.join(SRC_DIR, "yaml", "sensors", "derived_n2k.yaml")
    if os.path.isfile(path):
        return path

    sys.path.insert(0, HELPERS_DIR)
    from map_nmea_sensors import generate_derived_yaml

    print("derived_n2k.yaml is missing — generating it from the generic fallbacks")
    generate_derived_yaml({}, path)
    return path
