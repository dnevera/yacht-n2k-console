...
│       ├── dashboard/            # Dashboard header and sections (01_sensors, 02_wind, etc.)
...
│   ├── cards/
│   └── local-preview/
│       └── card-configs.js       # Auto-generated preview config for local-preview harness
```
...
- `deploy_dashboard.sh` — converts `build/dashboard-sailing.yaml` into JSON and uploads it into the `homeassistant` docker container's
  `.storage/lovelace.dashboard_sailing`, backing up the previous version
  on the remote host first, then **restarts Home Assistant**.
