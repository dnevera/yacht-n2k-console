...
2. **Форекаст — маленькие повёрнутые стрелки.** `forecast_dir`/`forecast_wind`
   open-meteo — фиксированный МАССИВ в атрибутах одного сенсора, а не
   recorder-история по часам — `windrose-card` (как и apexcharts-card/
   compass-card) не может это отрисовать. Оставлена компактная строка из
   8 `type: tile` (`type: grid`, `columns: 8`), по часу от текущего момента
   (`+0h`...`+21h`), маленькая (`--tile-icon-size: 22px`, а не дефолтный
   крупный размер) иконка `mdi:navigation`, повёрнутая через `card_mod` по
   `forecast_dir[найденный_текущий_час + N]` (индекс ищется относительно
   `utcnow()`, т.к. массив всегда начинается с полуночи UTC, а не с "сейчас").
...
`transform` для целой серии, поэтому для направления взята отдельная
windrose-карточка, а не попытка "натянуть" это на линейный график.
